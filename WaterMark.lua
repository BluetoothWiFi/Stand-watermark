...
if not filesystem.exists(filesystem.scripts_dir() .. '/watermark/icon.png') then
    ...
    async_http.init('raw.githubusercontent.com', '/BluetoothWiFi/Stand-watermark/main/stand/watermark/stand_icon.png', function(req)
		if not req then
			util.toast("Failed to download watermak/stand_icon.png, please download it manually.\nThe link is copied in your clipboard.")
            util.copy_to_clipboard("https://github.com/IBluetoothWiFi/Stand-watermark/blob/main/stand/watermark/stand_icon.png", true)
            return 
        end
...
menu.list_select(menu.my_root(), 'First Label', {}, 'Change the first label in the watermak', {'Disable', 'Stand', 'Version', 'FemboyEdition', 'Сок', 'OwO'}, settings.show_firstl, function (val)
...
menu.toggle(menu.my_root(), 'Date', {}, 'Shows OS time in the watermark', function(val)
